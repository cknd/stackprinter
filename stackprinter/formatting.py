...
from stackprinter.frame_formatting import ColorfulFrameFormatter, FrameFormatter
from stackprinter.utils import get_ansi_tpl, match
...
def format_summary(frames, style="plaintext", source_lines=1, reverse=False, **kwargs):
    """
    ...
    minimal_formatter = get_formatter(style=style, source_lines=min_src_lines, show_signature=False, show_vals=False)
...
def format_stack(
    ...
    suppressed_vars=[],
    truncate_suppressed_frames=False,  # TODO: check variable name!
):
    """
    ...
    reduced_formatter = get_formatter(
        style=style,
    ...
    verbose_formatter = get_formatter(
        style=style,
    ...
    last_boring_frame_fi = None
    boring_frames_count: int = 0
    for frame in frames:
        fi = ex.get_info(frame, suppressed_vars=suppressed_vars)
        is_boring = match(fi.filename, suppressed_paths)
        if is_boring:
            if truncate_suppressed_frames and last_boring_frame_fi is not None:
                # Already in a boring sequence and set to hide => skip frame
                formatter = None
            else:
                # Not set to hide, or this is the first boring frame in sequence => display frame
                if parent_is_boring:
                    formatter = minimal_formatter
                elif truncate_suppressed_frames:
                    # Use minimal format for first boring frame
                    formatter = minimal_formatter
                else:
                    formatter = reduced_formatter

            # Remember last boring frame in case it is the last in the sequence of boring frames and
            # should be displayed after the '... suppressed frames ...' message.
            # Also increment count of boring frames in the sequence.
            last_boring_frame_fi = fi
            boring_frames_count += 1
        else:
            if last_boring_frame_fi:
                # We are coming out of a boring sequence, print 'suppressed frames' line if sequence count > 2 (first and
                # last boring frames are shown), then print last boring frame.
                if boring_frames_count > 2:
                    frame_msgs.append(f"[--- {boring_frames_count - 2} suppressed frames truncated ---]\r\n")
                frame_msgs.append(minimal_formatter(last_boring_frame_fi))

            formatter = verbose_formatter

            # Reset boring sequence tracking variables
            last_boring_frame_fi = None
            boring_frames_count = 0

        parent_is_boring = is_boring
        if formatter is not None:
            # formatter is None when we are hiding boring frames, else append normally
...
def format_exc_info(
    ...
    add_summary="auto",
    reverse=False,
    suppressed_exceptions=[KeyboardInterrupt],
    suppressed_vars=[],
    **kwargs,
):
    """
    ...
    try:
        ...
        parts = []
        if tb:
            frameinfos = [ex.get_info(tb_, suppressed_vars=suppressed_vars) for tb_ in _walk_traceback(tb)]
            if suppressed_exceptions and issubclass(etype, tuple(suppressed_exceptions)):
                summary = format_summary(frameinfos, style=style, reverse=reverse, **kwargs)
                parts = [summary]
            else:
                whole_stack = format_stack(frameinfos, style=style, reverse=reverse, **kwargs)
        ...
        our_tb = traceback.format_exception(exc.__class__, exc, exc.__traceback__, chain=False)
